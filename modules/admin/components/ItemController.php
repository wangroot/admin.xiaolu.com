...
class ItemController extends Controller
{
    ...
    protected function getItems($id)
    {
        ...
        foreach ($manager->getPermissions() as $name) {
            ...
            $avaliable[$key.'_'.$description] = $key[0] == '/' ? 'route' : 'permission';
        }
        $assigned = [];
        foreach ($manager->getChildren($id) as $item) {
            $description = empty($item->description)?'未设置':$item->description;
            $assigned[$item->name.'_'.$description] = $item->type == 1 ? 'role' : ($item->name[0] == '/' ? 'route' : 'permission');
            unset($avaliable[$item->name.'_'.$description]);
        }
        unset($avaliable[$id]);
        return[
            'avaliable' => $avaliable,
